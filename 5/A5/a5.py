...
def bar(param: str) -> str:
    length = len(param)
    return f"bar: {param}, {length}"
    

def foobar(callables: Sequence[Callable[[str], str]], strings: Sequence[str]) -> None:
    results = []
    for i in range(2):
        results.append(callables[i](strings[i]))
    print(results)
